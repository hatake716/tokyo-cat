...
東京を野良猫として歩く Android 向け3D探索ゲーム。**0.4.0-dev / 開発版**。

丸の内・新宿・渋谷・浅草・秋葉原を、PLATEAUの実在する建物・地形データで表示します。観光、猫への挨拶、撮影とアルバムを実装しています。猫は、独自の体・毛・動きと、CC BY 4.0の公開モデルから改変した顔を組み合わせた10区分のスキン付きGLBです。**猫の写実表現、毛の質感、品種ごとの解剖学的精度・モーションは引き続き改良が必要です。完成したフォトリアル作品という位置づけではありません。**

## 0.4の顔と質感

まぶた・頬・鼻・口元・耳の内側がつながる顔メッシュへ置き換えました。顔の短い毛、鼻と口元の細部、曲線のひげ、虹彩、角膜の反射を調整しています。顔の色画像には元素材から生成AIで補整したテクスチャを使用し、形状・法線・まぶたの変形には作者が公開した素材を使用します。

顔の制作元は **Bicolor Cat / kenchoo**、原作 **Fripouille / guillaume bolis**、ライセンスは **CC BY 4.0** です。アプリと配布物に作者、配布ページ、ライセンス、変更内容を表示します。[顔の仕様・出典・制約](docs/FACE.md)。

体・顔・毛は26関節に追従。既存のIdle / Walk / Trot / Greet / Sitに、まぶたを変形させるBlinkを追加しました。下毛と差し毛の2層、移動距離に合わせた歩行、呼吸、耳、5節の尾を維持しています。[毛の仕様](docs/FUR.md)。

撮影モードに「顔をアップ」を追加しました。[Androidの実際の表示](docs/verification/cat-review.md)に正面・横顔・まばたきと全10種を記録しています。
...
- 撮影モードでは構図、距離、座位、顔のアップを調整。画像はアプリ内アルバムへ保存。
...
- 体・毛・移動は独自制作、顔はライセンスを確認した公開モデルの改変です。動画からの写真測量やモーションキャプチャではありません。10区分の体格・毛色の差を実装していますが、実写同等の毛並みや品種の正確な個体再現は未達です。
...
ビルド後に `python3 tools/package_dev.py` を実行すると、`artifacts/0.4.0-dev/` にAPK、猫モデルZIP、SHA256SUMS.txtを作成します。
