...
東京を野良猫として歩く Android 向け3D探索ゲーム。**0.3.0-dev / 開発版**。
...
10種すべてに下毛と差し毛の2層を追加しました。長毛では約9,000、短毛では約5,600の根元サンプルから毛束を作り、目と鼻を避け、頬・胸・お腹・しっぽを厚くしています。毛先が透ける湾曲した毛束と柔らかな照明で、輪郭と毛の重なりを表現します。耳にも短い毛を追加し、頬の丸み、瞳孔、目の光を調整しました。

毛は体表の24関節に追従し、歩く・座る・挨拶する動きに合わせて変形します。遠くの猫の毛と関節更新を段階的に減らし、近くの猫の毛並みを優先します。個々の毛の物理シミュレーションではありません。[毛の仕様と描画の説明](docs/FUR.md)、[Androidの実際の表示](docs/verification/cat-review.md)を記録しています。

0.2で改良した歩幅と移動距離の対応、まばたき、呼吸、耳、5節の尾も維持しています。GLBのIdle / Walk / Trot / Greet / Sitはゲームと共通の姿勢計算から生成します。
...
ビルド後に `python3 tools/package_dev.py` を実行すると、`artifacts/0.3.0-dev/` にAPK、猫モデルZIP、SHA256SUMS.txtを作成します。
